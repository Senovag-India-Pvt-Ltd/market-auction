...
@Service
@Slf4j
public class SeedMarketReelerAuctionService {
...
//        try {
//            JwtPayloadData token = marketAuctionHelper.getSeedMarketReelerAuthToken(reelerBidRequest);
//            entityManager = entityManagerFactory.createEntityManager();
//            StoredProcedureQuery procedureQuery = entityManager
//                    .createStoredProcedureQuery("SUBMIT_BID_EXCEPTIONAL_SLOT_FOR_SEED_MARKET");
...
//            procedureQuery.setParameter("AuctionDate", reelerBidRequest.getAuctionDate());
...
//    }
    ...
    public ResponseEntity<?> submitbidSP(ReelerBidRequest reelerBidRequest) {
        ...
        try {
            ...
            // Check if buyer type is "Reeling" for the given allottedLotId using the repository method
            String buyerType = lotGroupageRepository.findBuyerTypeByAllottedLotIdAndAuctionDateAndMarketId(
                    reelerBidRequest.getAllottedLotId(),
                    reelerBidRequest.getAuctionDate(),
                    reelerBidRequest.getMarketId()
            );

            if (buyerType == null) {
                ValidationMessage validationMessage = new ValidationMessage(
                        MessageLabelType.NON_LABEL_MESSAGE.name(),
                        "Bid submission is not allowed as the buyer type is not Reeling.",
                        "-1"
                );
                rw.setErrorCode(-1);
                rw.setErrorMessages(List.of(validationMessage));
            ...
            // Proceed with stored procedure call if the buyer type is "Reeling"
            entityManager = entityManagerFactory.createEntityManager();
            StoredProcedureQuery procedureQuery = entityManager
                    .createStoredProcedureQuery("SUBMIT_BID_EXCEPTIONAL_SLOT_FOR_SEED_MARKET");
            procedureQuery.registerStoredProcedureParameter("UserId", String.class, ParameterMode.IN);
            ...
            log.error("Error while submitting the bid for the Request: " + reelerBidRequest + " error is: " + ex);
            return marketAuctionHelper.retrunIfError(rw, "Error occurred while submitting bid.");
        } finally {
            if (entityManager != null && entityManager.isOpen()) {
                entityManager.close();
...
//        procedureQuery.registerStoredProcedureParameter("Success", Integer.class, ParameterMode.OUT);
            ...
            procedureQuery.setParameter("AuctionDate", lotStatusRequest.getAuctionDate());
            procedureQuery.execute();
//        String error = (String)procedureQuery.getOutputParameterValue("Error");
//        Object success = procedureQuery.getOutputParameterValue("Success");
        ...
        try{
            ...
            if(reelerLotHighestAndHisBidList.size()>0) {
                ...
                if (ra != null) {
                    ...
                    Object[][] ldrDetails = reelerAuctionRepository.getLotBidDetailResponse(lotStatusRequest.getAllottedLotId(), lotStatusRequest.getAuctionDate(), lotStatusRequest.getMarketId());
...
//    }
    ...
    public ResponseEntity<?> acceptReelerBidForGivenLot(ReelerBidAcceptRequest lotStatusRequest) {
        ...
        try {
            ...
            Lot lot = lotRepository.findByMarketIdAndAllottedLotIdAndAuctionDate(lotStatusRequest.getMarketId(), lotStatusRequest.getAllottedLotId(), lotStatusRequest.getAuctionDate());
//            if (!Util.isNullOrEmptyOrBlank(lot.getStatus())) {
//                throw new ValidationException(String.format("expected Lot status is blank but found:%s for the allottedLotId: %s",lot.getStatus(),lot.getAllottedLotId()));
//            }
            if (lot == null) {
                throw new ValidationException("Lot not found for the given marketId and allottedLotId.");
            }

// Check if the lot status is not 'weighmentcompleted'
            if (!"weighmentcompleted".equalsIgnoreCase(lot.getStatus())) {
                throw new ValidationException(String.format(
                        "Expected Lot status is 'weighmentcompleted' but found: %s for the allottedLotId: %s",
                        lot.getStatus(),
                        lot.getAllottedLotId()
                ));
            }
//            int acceptStartChecker = marketAuctionHelper.checkCurrentAuctionAccept(lotStatusRequest.getMarketId());
//            ReelerAuction reelerAuction = reelerAuctionRepository.getHighestBidForLot(lotStatusRequest.getAllottedLotId(), lotStatusRequest.getMarketId(), Util.getISTLocalDate(), acceptStartChecker);
...
//            getHighestBidPerLotDetails(lotStatusRequest1);
            ReelerAuction reelerAuction = new ReelerAuction();
                ...
                if(reelerLotHighestAndHisBidList.size()>0) {
                    ...
                    if (reelerAuction != null) {
                        Object[][] reelerDetails = reelerAuctionRepository.getReelerDetailsForHighestBidWithReelingNumber(reelerAuction.getId());
                        if (reelerDetails == null || reelerDetails.length == 0) {
                            rw.setErrorCode(-1);
                            rw.setErrorMessages(List.of("No reeler found please check for allotedLotId:" + lotStatusRequest.getAllottedLotId() + "and market: " + lotStatusRequest.getMarketId()));
            ...
            if (reelerAuction != null) {
                Lot l = lotRepository.findByMarketIdAndAllottedLotIdAndAuctionDate(reelerAuction.getMarketId(), reelerAuction.getAllottedLotId(),reelerAuction.getAuctionDate());
                l.setStatus(LotStatus.ACCEPTED.getLabel());
                l.setReelerAuctionId(reelerAuction.getId());
                l.setBidAcceptedBy(token.getUsername());
                reelerAuction.setStatus(LotStatus.ACCEPTED.getLabel());
